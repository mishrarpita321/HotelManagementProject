...
import { addAdminParkings, editAdminParkings } from 'src/store/admin/parkings';
...
const EditParkingDialog = ({ setShowEditDialog, editRow, setEditRow }) => {
    ...
    useEffect(() => {
        if (editRow) {
            setValue('vehicleType', editRow?.vehicleType);
    ...
    const onSubmit = (data) => {
        ...
        let dataWithId = { ...data, id: editRow.id }

        dispatch(editAdminParkings(dataWithId)).then((data) => {
            console.log(data)
...
export default EditParkingDialog;
